...
function run_true_shear_deformation(ls::Local_structure, stress_strain_output_file, cntr_theta::Int64) #FIXME
	...
	#video_filename = ls.output_path*"videos/deform_"*string(ls.cntr_x)*"_"*string(ls.cntr_y)*"_"*string(ls.theta) #FIXME
	video_filename = ls.output_path*"videos/deform_"*string(ls.cntr_x)*"_"*string(ls.cntr_y)*"_"*string(cntr_theta)
	# stress strain output file
	#
	deformation_steps = 1000
	#
	molcalc = Jumol.Calc(ls.molstruc)
	Jumol.initialize_potential(molcalc, 33) #FIXME check type of potential
	#
	eps = 1.0e-10
	...
	delta_stress = 1.0
	cntr_step = 1
	anim = @animate while delta_stress > 0.0
		#
		# step number
		println("####")
...
function run_true_shear_deformation_lmp(ls::Local_structure, lmp_executable::String, stress_strain_output_file)
	# get the number of atoms in r_free
	cntr_atom_free = 0
	for atom in ls.molstruc.atom_list
		if atom.group == 0
	...
	write(stress_strain_output_file, "# init_sigma_xx, init_sigma_yy, init_sigma_xy, num_steps, sigma_xx, sigma_yy, sigma_xy\n")
	...
	# video output filename
	video_output_filename = ls.output_path*"videos/deform_"*string(ls.cntr_x)*"_"*string(ls.cntr_y)*"_"*string(ls.theta)
	if ls.save_def_vid
