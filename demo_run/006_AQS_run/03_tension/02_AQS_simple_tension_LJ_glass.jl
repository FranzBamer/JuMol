...
## minimizer data and initial minimization
alpha_min = 1.0e-1
eps = 1.0e-7
...
fname = join([output_file, "output_hist_simple_tension.lammpstrj"])
Jumol.open_file(molstruc.reader, fname, "w")
stress_strain_file = open(join([output_file, "stress_strain_simple_tension.dat"]), "w")
anim = @animate for i in 1:num_steps
    ...
    Minimizer.acc_factor = 1.001
    Jumol.run_cg(Minimizer,max_num_steps,eps)
    ## update distances again
    Jumol.update_distances(molstruc)
    Jumol.update_neighbor_lists(molstruc)
    ## box relax / minimize
    Box_rel = Jumol.Box_relax(molcalc,Deformer,100,eps,1.0e-8)
    Jumol.relax_direction(Box_rel,2,delta_x/1.5)
...
gif(anim, join([output_file,"video_deform_simple_tension.gif"]), fps = 25)
